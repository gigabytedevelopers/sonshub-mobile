...
public class DownloadFileAdapter extends RecyclerView.Adapter<DownloadFileAdapter.ViewHolder> {
    ...
    public DownloadFileAdapter(@NonNull final ActionListener actionListener) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        ...
        holder.progressTextView.setText("downloaded " + context.getString(R.string.percent_progress, progress));

        if (downloadData.eta == -1) {
            holder.timeRemainingTextView.setText("");
        } else {
            holder.timeRemainingTextView.setText(DownloadUtils.getETAString(context, downloadData.eta));
        }

        if (downloadData.downloadedBytesPerSecond == 0) {
            holder.downloadedBytesPerSecondTextView.setText("");
        } else {
            holder.downloadedBytesPerSecondTextView.setText(DownloadUtils.getDownloadSpeedString(context, downloadData.downloadedBytesPerSecond));
        }
        ...
        switch (status) {
            ...
            case FAILED: {
                holder.actionButton.setImageResource(
                        com.gigabytedevelopersinc.apps.sonshub.downloader.fetch2.R.drawable.fetch_notification_retry
                );
